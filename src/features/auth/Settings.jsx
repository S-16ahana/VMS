...
export default function Settings() {
  ...
  // NEW: wait for authentication state so we don't redirect prematurely
  ...
  const refreshUsers = () => {
    ...
    const merged = [
      ...BUILT_IN.filter(
        (b) =>
          !stored.find((su) => su.email.toLowerCase() === b.email.toLowerCase())
      ),
      ...stored,
    ];
    setUsers(merged);
  ...
  const saveStoredUsers = (list) => {
    ...
    const merged = [
      ...BUILT_IN.filter(
        (b) =>
          !list.find((su) => su.email.toLowerCase() === b.email.toLowerCase())
      ),
      ...list,
    ];
    setUsers(merged);
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Typography variant="body2" sx={{ mb: 2 }}>
        Signed in as <strong>{userName || "Admin"}</strong>. Create users and
        manage passwords (demo/local).
      </Typography>

      <Stack direction="row" spacing={2} sx={{ mb: 2 }}>
        <ReusableButton onClick={() => setCreateOpen(true)}>
          Create New User
        </ReusableButton>
      </Stack>
