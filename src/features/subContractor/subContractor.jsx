...
const SubContractor = () => {
  ...
  useEffect(() => {
    dispatch(
      fetchSubcontractorEntries({ year: selectedYear, month: selectedMonth })
    );
  ...
  const scVendors = useMemo(
    () => vendors.filter((v) => v.type === "SC"),
    [vendors]
  );

  const formatCurrency = useCallback(
    (a) => (a ? `₹${a.toLocaleString("en-IN")}` : "₹0"),
    []
  );
  ...
  const handleSave = useCallback(
    async (entryData) => {
      if (editingEntry) {
        await dispatch(
          updateSubcontractorEntry({ id: editingEntry.id, ...entryData })
        );
      } else {
        await dispatch(
          createSubcontractorEntry({
      ...
      ...[
        ...
        muiTableBodyCellProps: {
          sx: {
            ...
            whiteSpace: "nowrap",
            fontWeight:
              col.key === "net_total" || col.key === "payables" ? 700 : 500,
            color:
              col.key === "gst_amount"
                ? "orange.main"
                : col.key === "tds"
        ...
        Cell: ({ row }) => {
            ...
            return (
              <Box
                sx={{
                  textAlign: "right",
                  fontWeight: 700,
                  color: v > 0 ? "error.main" : "success.main",
                }}
              >
                {formatCurrency(v)}
              </Box>
          ...
          return (
            <Box sx={{ textAlign: "right" }}>
              {formatCurrency(row.original[col.key])}
            </Box>
          );
        },
      })),
      ...
      >
        <Suspense fallback={<Loader message="Loading table..." />}>
          <ReusableTable
            ...
            options={{
              getRowId: (row) =>
                row.id ?? `${row.vendor_code}-${row.particular ?? ""}`,
              maxHeight: "60vh",
              ...
              muiTableProps: {
                sx: {
                  tableLayout: "fixed",
                  "& .MuiTableCell-root": { fontSize: "0.875rem", px: 1 },
                },
              },
            }}
          />
