...
export default function MonthlyEntryModal({ open, onClose, type = 'SC', vendors = [], initialData = {}, onSave }) {
  ...
  useEffect(() => {
    if (!open) return;
    const defaults = {
      vendor_code: '', particular: '', bill_type: 'H/Soft Copy', gross_amount: '', gst_rate: 5, tds_rate: 1,
    ...
    if (initialData.vendor_code) {
      ...
      if (v && defaults.particular) {
        ...
        if (defaults.particular.startsWith(wt)) {
          const rest = defaults.particular.slice(wt.length).replace(/^\s*-\s*/, '');
          setNarration(rest);
          setFormData(prev => ({ ...prev, particular: rest ? `${wt} - ${rest}` : wt }));
        } else {
          setNarration(defaults.particular || '');
          setFormData(prev => ({ ...prev, particular: defaults.particular || '' }));
        }
      }
    } else {
  ...
  const calculatedValues = useMemo(() => {
    const normalized = {
      ...formData,
      gross_amount: formData.gross_amount === '' ? '' : Number(formData.gross_amount) || 0,
      gst_rate: formData.gst_rate === '' ? '' : Number(formData.gst_rate) || 0,
      tds_rate: formData.tds_rate === '' ? '' : Number(formData.tds_rate) || 0,
      debit_deduction: formData.debit_deduction === '' ? 0 : Number(formData.debit_deduction) || 0,
      gst_hold: formData.gst_hold === '' ? 0 : Number(formData.gst_hold) || 0,
      other_deductions: formData.other_deductions === '' ? 0 : Number(formData.other_deductions) || 0,
      advances: formData.advances === '' ? 0 : Number(formData.advances) || 0,
      part_paid: formData.part_paid === '' ? 0 : Number(formData.part_paid) || 0,
    };

    if (normalized.gross_amount === '' || normalized.gross_amount == null) return {};
    try {
  ...
  }, [formData.gross_amount, formData.gst_rate, formData.tds_rate, formData.debit_deduction, formData.gst_hold, formData.other_deductions, formData.advances, formData.part_paid]);
    ...
    if (selectedVendor && fetchedPart) {
      ...
      if (hyphenIndex >= 0) {
        ...
        if (raw.length < left.length) {
          ...
          const trailing = raw.slice(left.length).replace(/^\s*-\s*/, '').trim();
          setNarration(trailing);
          setField('particular', trailing ? `${left} - ${trailing}` : left);
        } else {
          const trailing = raw.replace(left, '').replace(/^\s*-\s*/, '').trim();
          setNarration(trailing);
          setField('particular', trailing ? `${left} - ${trailing}` : left);
        }
      }
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    if (!selectedVendor) { setVendorLookupError('Please select a valid vendor'); return; }
    if (formData.gross_amount === '' || formData.gross_amount == null) { alert('Please fill in Gross Amount'); return; }
    if (!combinedParticular) { alert('Please fill in Particular'); return; }
  ...
  }, [selectedVendor, formData, combinedParticular, calculatedValues, onSave, onClose]);
      ...
      <Box sx={modalStyle}>
        ...
        <Box sx={{ maxHeight: 'calc(90vh - 140px)', overflow: 'auto', p: 3 }} component="form" onSubmit={handleSubmit}>
          <Stack spacing={3}>
                ...
                <TextField
                  ...
                  InputProps={{
                    endAdornment: selectedVendor ? (
                      <InputAdornment position="end"><Chip label="Found" size="small" /></InputAdornment>
                    ) : undefined
                  }}
                />
            ...
            <Box>
              ...
              <Stack spacing={2}>
                ...
                <TextField
                  ...
                  placeholder={type === 'SC' ? (fetchedPart ? `${fetchedPart} - narration (optional)` : 'Work description') : 'Vehicle/Equipment details'}
                  required
                  helperText={selectedVendor ? `Fetched: "${fetchedPart}". Add narration after " - " (optional).` : ''}
                />

                <FormControl size="small">
                  <InputLabel>Bill Type</InputLabel>
                  <Select value={formData.bill_type || 'H/Soft Copy'} label="Bill Type" onChange={handleInputChange('bill_type')}>
            ...
            <Box>
              <Typography variant="subtitle1" sx={{ fontWeight: 600, display: 'flex', alignItems: 'center', gap: 1 }} gutterBottom><CalculateIcon fontSize="small" /> Financial Calculations</Typography>
              <Stack spacing={2}>
                <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 2 }}>
                  <TextField label="Gross Amount" type="number" size="small" required value={formData.gross_amount || ''} onChange={handleNumberChange('gross_amount')} InputProps={{ startAdornment: <InputAdornment position="start">₹</InputAdornment> }} />

                  <TextField label="GST Rate" size="small" value={formData.gst_rate === '' || formData.gst_rate == null ? '' : String(formData.gst_rate)} onChange={handlePercentChange('gst_rate')} placeholder="e.g. 5 or 5%" InputProps={{ endAdornment: <InputAdornment position="end">%</InputAdornment> }} />
                </Box>

                <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr 1fr' }, gap: 2 }}>
                  <TextField label="GST Amount" size="small" value={formatCurrency(calculatedValues.gst_amount)} InputProps={{ readOnly: true }} sx={{ '& input': { color: 'orange.main', fontWeight: 500 } }} />
                  <TextField label="Total Amount" size="small" value={formatCurrency(calculatedValues.total_amount)} InputProps={{ readOnly: true }} sx={{ '& input': { color: 'primary.main', fontWeight: 600 } }} />
            ...
            <Box>
              ...
              <Stack spacing={2}>
                <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr 1fr' }, gap: 2 }}>
                  <TextField label="TDS" size="small" value={formatCurrency(calculatedValues.tds)} InputProps={{ readOnly: true }} sx={{ '& input': { color: 'error.main', fontWeight: 500 } }} />
                  <TextField label="Debit/Deduction" type="number" size="small" value={formData.debit_deduction || ''} onChange={handleNumberChange('debit_deduction')} InputProps={{ startAdornment: <InputAdornment position="start">₹</InputAdornment> }} />
                  <TextField label="Retention (5%)" size="small" value={formatCurrency(calculatedValues.retention)} InputProps={{ readOnly: true }} sx={{ '& input': { color: 'warning.main', fontWeight: 500 } }} />
                </Box>

                <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, gap: 2 }}>
                  <TextField label="GST Hold" type="number" size="small" value={formData.gst_hold || calculatedValues.gst_amount || ''} onChange={handleNumberChange('gst_hold')} InputProps={{ startAdornment: <InputAdornment position="start">₹</InputAdornment> }} />
                  <TextField label="Other Deductions" type="number" size="small" value={formData.other_deductions || ''} onChange={handleNumberChange('other_deductions')} InputProps={{ startAdornment: <InputAdornment position="start">₹</InputAdornment> }} />
                </Box>
              </Stack>
            </Box>
            ...
            <Box>
              ...
              <Stack spacing={2}>
                <TextField label="NET TOTAL" size="small" value={formatCurrency(calculatedValues.net_total)} InputProps={{ readOnly: true }} sx={{ '& input': { color: 'success.main', fontWeight: 700, fontSize: '1.05rem' } }} />

                <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr 1fr' }, gap: 2 }}>
                  <TextField label="Advances" type="number" size="small" value={formData.advances || ''} onChange={handleNumberChange('advances')} InputProps={{ startAdornment: <InputAdornment position="start">₹</InputAdornment> }} />
                  <TextField label="Part Paid" type="number" size="small" value={formData.part_paid || ''} onChange={handleNumberChange('part_paid')} InputProps={{ startAdornment: <InputAdornment position="start">₹</InputAdornment> }} />
                  <TextField label="PAYABLES" size="small" value={formatCurrency(calculatedValues.payables)} InputProps={{ readOnly: true }} sx={{ '& input': { color: (calculatedValues.payables > 0 ? 'error.main' : 'success.main'), fontWeight: 700, fontSize: '1.05rem' } }} />
                </Box>

                <TextField label="Notes" value={formData.notes || ''} onChange={handleInputChange('notes')} multiline rows={2} size="small" placeholder="Optional notes or comments" />
              </Stack>
            </Box>

            {calculatedValues.payables < 0 && (
              <Alert severity="warning"><strong>Warning:</strong> Payable amount is negative. Please check advances and part paid amounts.</Alert>
            )}
        ...
        <Box sx={{ p: 3, borderTop: 1, borderColor: 'divider', display: 'flex', justifyContent: 'flex-end', gap: 2 }}>
          ...
          <Button onClick={handleSubmit} variant="contained" disabled={submitting || !selectedVendor}>{submitting ? 'Saving...' : initialData.id ? 'Update Entry' : 'Create Entry'}</Button>
        </Box>
      </Box>
    </Modal>
  );
