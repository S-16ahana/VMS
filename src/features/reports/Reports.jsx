...
const aggregateEntries = (entries, year) => {
  ...
  for (const e of entries || []) {
    ...
    rec.months[mn].retention += +(
      (e.retention || 0) + (e.other_deductions || 0)
    );
...
const buildReport = (vendors = [], entriesMap, type) => {
  return vendors
    ...
    .map((v) => {
      const agg = entriesMap.get(v.vendor_code) || {
        months: {},
        totalPayables: 0,
        totalGST: 0,
        totalRetention: 0,
      ...
      row.totalPayables = MONTH_NUMS.reduce(
        (s, m) => s + (agg.months[m]?.payables || 0),
        0
      );
      row.totalGST = MONTH_NUMS.reduce(
        (s, m) => s + (agg.months[m]?.gst || 0),
        0
      );
      row.totalRetention = MONTH_NUMS.reduce(
        (s, m) => s + (agg.months[m]?.retention || 0),
        0
      );
...
const createColumns = (type) => {
  const base = [
    {
      ...
      Cell: ({ row }) => (
        <Chip
          ...
          color={type === "SC" ? "primary" : "secondary"}
          size="small"
        />
      ),
    },
  ...
  const months = MONTH_NUMS.map((m, i) => ({
    accessorKey: m.toString(),
    header: MONTH_NAMES[i],
    size: 80,
    Cell: ({ row }) => (
      <Box sx={{ textAlign: "right", fontSize: "0.875rem" }}>
        {row.original[m] > 0 ? formatCurrency(row.original[m]) : "-"}
      </Box>
    ),
  }));
  ...
  const totals = [
    {
      ...
      Cell: ({ row }) => (
        <Box sx={{ textAlign: "right", fontWeight: 600, color: "error.main" }}>
          {formatCurrency(row.original.totalPayables)}
        </Box>
      ),
    },
    {
      ...
      Cell: ({ row }) => (
        <Box
          sx={{ textAlign: "right", fontWeight: 500, color: "warning.main" }}
        >
    ...
    {
      accessorKey: "totalRetention",
      header: "Retention & Others",
      size: 160,
      Cell: ({ row }) => (
        <Box sx={{ textAlign: "right", fontWeight: 500, color: "info.main" }}>
          {formatCurrency(row.original.totalRetention)}
        </Box>
      ),
    },
    {
...
export default function Reports() {
  ...
  const entriesMap = useMemo(
    () => aggregateEntries(reportEntries, selectedYear),
    [reportEntries, selectedYear]
  );

  const subcontractorData = useMemo(
    () => buildReport(vendors, entriesMap, "SC"),
    [vendors, entriesMap]
  );
  const hiringData = useMemo(
    () => buildReport(vendors, entriesMap, "HS"),
    [vendors, entriesMap]
  );
  ...
  const totals = useMemo(
    () =>
      currentData.reduce(
        (acc, r) => ({
          payables: acc.payables + r.totalPayables,
          gst: acc.gst + r.totalGST,
          retention: acc.retention + r.totalRetention,
          grand: acc.grand + r.grandTotal,
        }),
        { payables: 0, gst: 0, retention: 0, grand: 0 }
      ),
    [currentData]
  );
  ...
  const handleExport = (data, name) => {
    ...
    a.href = url;
    a.download = `${name}_${selectedYear}.csv`;
          ...
          <Button
            ...
            onClick={() =>
              handleExport(
                currentData,
                selectedTab === 0
                  ? "subcontractor_yearly_report"
                  : "hiring_yearly_report"
              )
            }
          >
            Export CSV
